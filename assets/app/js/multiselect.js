...
function populateAutocompleteList(select, query, dropdown = false) {
    ...
    if (result_size >= 1) {
        ...
        Object.entries(options_to_show).forEach(option => {
            const li = document.createElement("li");
            li.innerText = option[1];
            li.setAttribute('data-value', option[0]);
...
function getOptions(select) {
    ...
    const all_options = Array.from(select.querySelectorAll("option")).reduce((acc, el) => {
        acc[el.value] = el.innerText;
        return acc;
    }, {});

    // Get the options that are selected from the user
    const options_selected = Array.from(select.querySelectorAll("option:checked")).reduce((acc, el) => {
        acc[el.value] = el.innerText;
        return acc;
    }, {});
    ...
    Object.entries(all_options).forEach(option => {
        if (!Object.values(options_selected).includes(option[1])) {
            autocomplete_options[option[0]] = option[1];
        }
    });
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.addEventListener('click', (event) => {
        ...
        for (let i = 0; i < select.length; i++) {
            if (event) {
                const isClickInside = select[i].parentElement.parentElement.contains(event.target);
